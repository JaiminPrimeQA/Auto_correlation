...
function RunHealth({ title, health }: { title: string; health: Health }) {
  ...
  return (
    <div className="card p-4">
      ...
      {/* Transport health */}
      <div className="mt-3 text-[11px] font-semibold uppercase tracking-wide text-fg-subtle">
        Transport health
      </div>
      <div className="mt-1 flex flex-wrap gap-2 text-xs">
        {Object.entries(health.status_distribution).map(([k, v]) => (
          <span key={k} className="rounded-[10px] bg-surface2 px-2 py-0.5 text-fg">
            {k}: {v}
          </span>
        ))}
      </div>
      ...
      <div className="mt-1 space-y-1">
        {health.business_signals.map((s, i) => (
          <div key={i} className="rounded-[10px] bg-surface2 px-2 py-1 text-[11px] text-fg-muted">
            <span className="text-fg">#{s.execution_index + 1} {s.name}</span> · {s.kind.replace(/_/g, " ")}: {s.detail}
          </div>
        ))}
      </div>

      {health.blockers.map((b, i) => (
        <p key={i} className="mt-2 flex items-center gap-1 rounded-[10px] bg-danger-soft px-3 py-2 text-xs text-danger">
          <ProhibitIcon size={14} aria-hidden />
          {b}
        </p>
      ))}
      {health.warnings.map((w, i) => (
        <p key={i} className="mt-2 flex items-center gap-1 rounded-[10px] bg-warn-soft px-3 py-2 text-xs text-warn">
  ...
  return (
    <div className="space-y-3">
      {summary.warnings.map((w, i) => (
        <div key={i} className="flex items-center gap-1 rounded-[10px] bg-warn-soft p-3 text-xs text-warn">
          <WarningIcon size={14} aria-hidden />
          {w}
        </div>
      ))}
      ...
      {summary.blocked && (
        <div className="rounded-[10px] bg-danger-soft p-4 text-sm text-danger">
      ...
      {summary.alignment && (
        <div className="card flex flex-wrap gap-4 p-3 text-xs text-fg">
