...
Companion to [2026-09-24-postman-collection-execution-design.md](2026-09-24-postman-collection-execution-design.md). This is the original engineering brief; the design doc is the approved architecture derived from it. Where the two overlap, the design doc is authoritative on structure; this document is authoritative on process requirements (TDD discipline, error taxonomy, documentation deliverables).
...
Add a user-friendly input mode that accepts a Postman Collection v2.0/2.1 JSON file and an optional Postman environment JSON file. The server must discover required variables, ask the user only for missing values, run the collection twice with Newman 6.2.2 in isolated workers, generate baseline and comparison JSON reports internally, and submit those reports to the existing correlation pipeline.
